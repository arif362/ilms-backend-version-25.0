...
module Lms
  class Classifications
    ...
    delegate :classification, :action_type, to: :context
    ...
    def user_able
      staff_id = action_type == 'created' ? classification.created_by_id : classification.updated_by_id
    ...
    def create_classification
      Library.where.not(id: base_library.id)&.each do |library|
        ...
        url = action_type == 'created' ? URI("#{library_base_url}/service/classification/source") : URI("#{library_base_url}/service/classification/source/#{classification.id}")
        ...
        request.body = JSON.dump(classification_params)
