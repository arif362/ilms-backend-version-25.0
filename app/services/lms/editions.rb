...
module Lms
  class Editions
    ...
    delegate :edition, :action_type, to: :context
    ...
    def user_able
      staff_id = action_type == 'created' ? edition.created_by_id : edition.updated_by_id
    ...
    def create_edition
      Library.where.not(id: base_library.id)&.each do |library|
        ...
        url = action_type == 'created' ? URI("#{library_base_url}/service/edition") : URI("#{library_base_url}/service/edition/#{edition.id}")
        Rails.logger.info("action url: #{edition.id}")
        ...
        request.body = JSON.dump(edition_params)
