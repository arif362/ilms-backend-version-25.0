...
module Lms
  module InterLibraryTransferManage
    class CreateIntLibExtension
      include Interactor

      delegate :ile, :user_able, to: :context

      def call
        create_int_lib_extension
      end
      ...
      def library_base_url
        "#{ile.sender_library.ip_address}/api/library"
      end

      def create_int_lib_extension
        url = URI("#{library_base_url}/service/library/lender/extend/request/book/#{ile.library_transfer_order.transferable.id}")
        https = Net::HTTP.new(url.host, url.port)
        https.use_ssl = library_base_url.start_with?('https')
        request = Net::HTTP::Post.new(url)
        request['Authorization'] = "Bearer #{access_token(ile.sender_library)}"
        request['Content-Type'] = 'application/json'
        request.body = JSON.dump({ borrow_extended_end_date: ile.extend_end_date.strftime('%Y-%m-%d') })
