...
module Lms
  class Itemtypes
    ...
    delegate :item, :action_type, to: :context
    ...
    private
    def user_able
      staff_id = action_type == 'created' ? item.created_by_id : item.updated_by_id
    ...
    def create_item_type
      Library.where.not(id: base_library.id)&.each do |library|
        ...
        url = action_type == 'created' ? URI("#{library_base_url}/service/itemtype") : URI("#{library_base_url}/service/itemtype/#{item.id}")
        Rails.logger.info("action url: #{item.id}")
        ...
        request.body = JSON.dump(item_type_params)
