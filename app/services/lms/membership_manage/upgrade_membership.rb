  ...
  module MembershipManage
    class UpgradeMembership
      ...
      def upgrade_membership_request
        url = URI("#{library_base_url}/service/patron/update/category/#{membership_request.id}")
      ...
      def membership_request_params
        {
          ...
          profile_image: image_path(request_detail.membership_request.user.image),
      ...
      def identity_image
        request_detail.identity_type == 'nid' ? image_path(request_detail.nid_front_image) : image_path(request_detail.birth_certificate_image)
      end
    end
  end
end
